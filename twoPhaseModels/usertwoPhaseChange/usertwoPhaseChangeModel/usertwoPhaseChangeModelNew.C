...
Foam::autoPtr<Foam::usertwoPhaseChangeModel> Foam::usertwoPhaseChangeModel::New
(
...
)
{
    ...
    word modelType(usertwoPhaseChangeModels::usernoPhaseChange::typeName);

    if (twoPhaseChangeModelIO.typeHeaderOk<IOdictionary>(false))
    {
        IOdictionary(twoPhaseChangeModelIO).lookup
        (
            usertwoPhaseChangeModel::typeName
        ) >> modelType;
    }
    else
    {
    ...
    if (cstrIter == dictionaryConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown " << usertwoPhaseChangeModel::typeName<< " type "
            << modelType << nl << nl
    ...
    return autoPtr<usertwoPhaseChangeModel>(cstrIter()(mixture));
}
